...
while cap.isOpened():
    ...
    '''# Extract contours with 2 to 4 children
    selected_contours = []
    for i, contour in enumerate(contoursRect):
        if 0 <= count_children(hierarchy2[0], i) <= 4:
            selected_contours.append(contour)
    
    '''
    # Draw contours with 2 to 4 children on the black image
    cv2.drawContours(frame, selected_contours, -1, (255, 0, 255), 3)
    
    # Display the black image with contours
